...
if ! test -f "$finit_inp"; then
	...
	srun gmx_mpi grompp -f minim.mdp -c main.pdb -p topol.top -o enermin.tpr
...
if ! test -f "$finit2_inp"; then
	...
	# run enermin.tpr
	srun gmx_mpi mdrun -s enermin.tpr -cpo state_min.cpt -cpi state_min.cpt -cpt 2 -g md_min.log -o traj_min.trr -e ener_min.edr -c confout_min.gro  
	wait

	echo "begin generating min_noflex.tpr.."
	# generate enermin files with no -DFLEXIBLE
	srun gmx_mpi grompp -f min_noflex.mdp -c confout_min.gro -p topol.top -o min_noflex.tpr
	wait
...
if ! test -f "$fmin_inp"; then
	...
	# run enermin.tpr
	srun gmx_mpi mdrun -s min_noflex.tpr -cpo state_min2.cpt -cpi state_min2.cpt -cpt 2 -g md_min2.log -o traj_min2.trr -e ener_min2.edr -c confout_min2.gro  
	wait

	echo "begin generating nvt.tpr.."
	# generate nvt files
	srun gmx_mpi grompp -f nvt.mdp -c confout_min2.gro -p topol.top -o nvt.tpr 
	wait
...
if ! test -f "$fnvt_inp"; then
	...
	# run nvt.tpr
	srun gmx_mpi mdrun -s nvt.tpr -cpo state_nvt.cpt -cpi state_nvt.cpt -cpt 5 -g md_nvt.log -o traj_nvt.trr -e ener_nvt.edr -c confout_nvt.gro  
	wait

	echo "begin generating npt_berendsen.tpr.."
	# generate npt_berendsen files
	srun gmx_mpi grompp -f npt_berendsen.mdp -c confout_nvt.gro -p topol.top -o npt_berendsen.tpr -maxwarn 1
	wait
...
if ! test -f "$fnpt_inp"; then
	...
	srun gmx_mpi mdrun -s npt_berendsen.tpr -cpo state_npt_berendsen.cpt -cpi state_npt_berendsen.cpt -cpt 5 -g md_npt_berendsen.log -o traj_npt_berendsen.trr -e ener_npt_berendsen.edr -c confout_npt_berendsen.gro -notunepme 
	wait

	echo "begin generating npt_main.tpr.."
	# generate npt_berendsen files
	srun gmx_mpi grompp -f npt_main.mdp -c confout_npt_berendsen.gro -p topol.top -o npt_main.tpr
	wait
	...
	cp confout_npt_berendsen.gro trajfiles/confout_npt_berendsen.gro
        ...
        # run npt_main.tpr
        srun gmx_mpi mdrun -s npt_main.tpr -cpo state_npt_main.cpt -cpi state_npt_main.cpt -cpt 5 -g md_npt_main.log -o traj_npt_main.trr -e ener_npt_main.edr -c confout_npt_main.gro -notunepme 
        wait
